...
func TestTimeEntrySubmit_OnlyContractor(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/time-entries/"+entryID.String()+"/submit", nil)
...
func TestTimeEntryReject_ReasonRequired(t *testing.T) {
	db, mock := newSQLMock(t)
	cfg := testJWTConfig()
	userID := uuid.MustParse("11111111-1111-1111-1111-111111111111")
	orgID := uuid.MustParse("22222222-2222-2222-2222-222222222222")
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/time-entries/"+entryID.String()+"/reject", strings.NewReader(`{"reason":""}`))
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set("X-Organization-ID", orgID.String())
