...
func TestBulkApprove_ContractorForbidden(t *testing.T) {
	...
	body := `{"time_entry_ids":["44444444-4444-4444-4444-444444444444"]}`
	req := httptest.NewRequest(http.MethodPost, "/v1/time-entries/bulk-approve", strings.NewReader(body))
...
func TestListTimeEntries_InvalidCursor(t *testing.T) {
	db, mock := newSQLMock(t)
	cfg := testJWTConfig()
	userID := uuid.MustParse("11111111-1111-1111-1111-111111111111")
	orgID := uuid.MustParse("22222222-2222-2222-2222-222222222222")
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/time-entries?cursor=badcursor", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set("X-Organization-ID", orgID.String())
	rec := httptest.NewRecorder()
